...
- `&` в `shell` -запуск в фоне
- Как работает команда `cat` - содержимое файлов
- Как работает команда `clear` -очистка консоли
- Как работает команда `wc` - подсчет строк слов и символов в файле
...
- Как работает команда `grep` - поиск строк по шаблону в файлах      grep [опции] "шаблон" [файл...]
- Как работает редирект в `bash`
- Что такое `pipe` в `bash` - запуск нескольких файлов              file1|file 2
